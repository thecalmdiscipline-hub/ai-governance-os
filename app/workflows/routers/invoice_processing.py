...
router = APIRouter(
    ...
    tags=["Workflows"],
    dependencies=[Depends(require_module_access("invoice_processing"))],
)
...
def run(
    ...
    result = run_workflow("invoice_processing", {"input": body.input, "context": body.context, "user": current_user.username}, user_id=current_user.id, org_id=current_user.organization_id)
    return result
