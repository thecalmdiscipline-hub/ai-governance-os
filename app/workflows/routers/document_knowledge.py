...
router = APIRouter(
    prefix="/document-knowledge",
    tags=["Workflows"],
    dependencies=[Depends(require_module_access("document_knowledge"))],
)
...
def run(
    ...
    result = run_workflow("document_knowledge", {"input": body.input, "context": body.context, "user": current_user.username}, user_id=current_user.id, org_id=current_user.organization_id)
    return result
