...
router = APIRouter(
    ...
    tags=["Workflows"],
    dependencies=[Depends(require_module_access("marketing_automation"))],
)
...
def run(
    ...
    result = run_workflow("marketing_automation", {"input": body.input, "context": body.context, "user": current_user.username}, user_id=current_user.id, org_id=current_user.organization_id)
    return result
